...
export default function WeddingInvite() {
  ...
  const toggleMusic = async () => {
    ...
    try {
      ...
      console.warn("Autoplay bloqueado, interactúa con la página primero");
    }
  };
  ...
  return (
    <div className="text-center text-[#3e2f25] relative overflow-hidden bg-[#f6efe7]" style={{ fontFamily: "'Playfair Display', serif" }}>
      ...
      {/* FLORES ACUARELA PREMIUM */}
      <motion.img src="/images/flowers1.png" className="absolute top-0 left-0 w-72 opacity-70 pointer-events-none z-0" style={{ transform: `translateY(${scrollY * 0.1}px)` }} />
      <motion.img src="/images/flowers2.png" className="absolute top-1/3 right-0 w-64 opacity-70 pointer-events-none rotate-12 z-0" style={{ transform: `translateY(${scrollY * 0.15}px)` }} />
      <motion.img src="/images/flowers3.png" className="absolute bottom-0 left-10 w-60 opacity-70 pointer-events-none -rotate-12 z-0" style={{ transform: `translateY(${scrollY * 0.2}px)` }} />
      <motion.img src="/images/flowers4.png" className="absolute bottom-20 right-10 w-56 opacity-70 pointer-events-none rotate-180 z-0" style={{ transform: `translateY(${scrollY * 0.25}px)` }} />
      ...
      <button onClick={toggleMusic} className="fixed top-4 right-4 bg-white/80 backdrop-blur px-4 py-2 rounded-xl shadow-lg z-50 hover:scale-105 transition-transform duration-300">
      ...
      <section className="min-h-screen flex flex-col items-center justify-center p-6 relative overflow-hidden">
        <img src={portadaImg} className="absolute inset-0 w-full h-full object-cover opacity-50 pointer-events-none" style={{ transform: `scale(${1 + scrollY * 0.0003})` }} />
        <div className="absolute inset-0 bg-gradient-to-b from-[#00000060] to-[#00000030] pointer-events-none"></div>

        <motion.div variants={fadeUp} initial="hidden" animate="visible" transition={{ duration: 1.2, ease: "easeOut" }} className="relative z-20 text-white">
          <h1 className="text-7xl md:text-9xl font-light tracking-[0.4em]" style={{ fontFamily: "Great Vibes" }}>J & V</h1>
      ...
      <motion.section variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true }} transition={{ duration: 0.8, ease: "easeOut" }} className="p-8 max-w-2xl mx-auto">
        <div className="bg-white/80 rounded-3xl shadow-xl p-8 border border-[#d6c3a3]">
          ...
          <div className="mt-6">
            ...
            <a href="https://maps.app.goo.gl/o9f6TPctt82EVftp6" target="_blank" rel="noopener noreferrer" className="underline">Ver ubicación</a>
          </div>
          <div className="mt-6">
            <p className="font-semibold">Recepción</p>
            <p>9:00 PM - Finca Hotel Los Guacamayos</p>
            <a href="https://maps.app.goo.gl/m123xDgmpxXahdsF6" target="_blank" rel="noopener noreferrer" className="underline">Ver ubicación</a>
          </div>
        </div>
      </motion.section>
      ...
      <motion.section variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true }} transition={{ duration: 0.8, ease: "easeOut" }} className="p-10">
        ...
        <div className="bg-white/80 rounded-3xl shadow-xl p-6 border border-[#d6c3a3] inline-block">
          ...
          <a href="https://docs.google.com/forms/d/e/1FAIpQLScVhCEAtqJb-zwlF6PZwAKj8pcUl2U1CXqVEfG0I6OKxVZpJA/viewform?usp=publish-editor" target="_blank" rel="noopener noreferrer" className="inline-block px-8 py-4 rounded-xl font-semibold text-white shadow-lg bg-[#C68642] hover:bg-[#a86b2d] hover:scale-105 transition-all duration-300">
            Confirmar asistencia aquí
          </a>
          <div className="mt-4">
            <a href="https://wa.me/573234017625" target="_blank" rel="noopener noreferrer" className="underline">Confirmar por WhatsApp</a>
          </div>
        </div>
      </motion.section>
